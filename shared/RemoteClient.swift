...
actor RemoteClient {
  ...
  private func invoke(operation: String, path: String? = nil, body: [String:Any]? = nil) async throws -> Data {
    ...
    guard data.count < 16_000_000, let wire = try JSONSerialization.jsonObject(with: data) as? [String:String], let encrypted = wire["box"].flatMap(Data.init(base64Encoded:)) else { throw RemoteError("Invalid host response.") }
