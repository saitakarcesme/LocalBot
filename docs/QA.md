...
### Live terminal Activity — 2026-09-17

119/119 sequential runtime tests and the 269+13 inventory audit pass. Real sandboxed stdout/stderr reach persisted Activity while the task and terminal call are still running; cancellation preserves the captured output. A background process_start record updates while inference waits, independently of process_poll's once-only output consumption. Tests also cover quiet trailing bursts, split UTF-8 output, cleanup with no late writes, process ownership, cancellation/lifetime/output bounds and existing permissions. Updates use leading/trailing 500 ms batching; live terminal tails are capped at 16,000 characters and session tails at 12,000 (further shortened when JSON escaping requires it). No native build, installation or live popover interaction was performed here.
