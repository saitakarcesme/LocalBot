...
const command = 'read value; printf "received:%s" "$value"';
const input = 'LocalBot stdin verified\n';
const conversation = await api('/conversations', { title: 'Süreç oturumu doğrulaması', members: ['coder'] });
const task = await api('/messages', { conversationId: conversation.id, content: `Süreç araçlarını gerçekten doğrula. process_start ile tam olarak şu komutu başlat: ${command}\nDönen sessionId ile process_input kullan; text tam olarak ${JSON.stringify(input)}, end "true" olsun. Ardından process_poll ile exited durumunu ve çıkış kodunu oku. Çıktı received:LocalBot stdin verified ve exitCode 0 ise kısa Türkçe sonuç ver. Başka komut veya dosya işlemi yapma.` });
...
try {
  ...
  while (Date.now() < deadline) {
    ...
    if (!['queued', 'running', 'awaiting_approval'].includes(current.status)) {
      ...
      assert.equal(polls.map(p => p.output).join(''), 'received:LocalBot stdin verified');
      done = true;
      console.log(JSON.stringify({ verified: true, conversationId: conversation.id, actions: actions.map(a => a.name), output: 'received:LocalBot stdin verified', exitCode: 0 }));
      break;
    }
