...
const conversation=await api('/conversations',{title:'Proje konuşmasını oku',members:['researcher'],projectId:project.id,automatic:false});
const task=await api('/messages',{conversationId:conversation.id,content:`Mira, read_history aracını conversation_id ${source.id} ile kullan. Bu kaynak sohbetin mesajlarını okuyup projede daha önce neler yapıldığını kısa Türkçe özetle. Başka araç kullanma.`});
let done=false;
try{const start=Date.now();while(Date.now()-start<180000){const snap=await api('/snapshot');const t=snap.tasks.find(t=>t.id===task.id);if(!['queued','running','awaiting_approval'].includes(t.status)){
 ...
 const results=actions.map(a=>JSON.parse(a.output));assert(results.some(r=>r.messages.length>0));for(const result of results)assert.equal(result.conversationId,source.id);
 const messages=await api('/messages?conversationId='+conversation.id);const reply=messages.filter(m=>m.role==='assistant').at(-1)?.content;assert(reply);console.log(JSON.stringify({verified:true,conversationId:conversation.id,messagesRead:results.reduce((n,r)=>n+r.messages.length,0),reply}));done=true;break;
