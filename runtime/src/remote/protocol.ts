...
export function remoteURL(value: string) {
  ...
  if ((url.protocol !== 'https:' && !(local && url.protocol === 'http:')) || url.username || url.password || url.search || url.hash || (url.pathname !== '/' && url.pathname !== '')) throw Error('Use an HTTPS connection address. HTTP is allowed only for loopback testing.');
  return url.origin;
}
export function parseLink(code: string): Link {
  const url = new URL(code.trim());
  if (url.protocol !== 'localbot:' || url.hostname !== 'pair') throw Error('This is not a LocalBot pairing code.');
...
export function encodeLink(link: Link) { return 'localbot://pair?data=' + Buffer.from(JSON.stringify(link)).toString('base64url'); }
const aad = (id: string, requestId: string, direction: string) => Buffer.from(`localbot.v1|${id}|${requestId}|${direction}`);
export function seal(key: string, id: string, requestId: string, direction: 'request' | 'response', value: unknown): string {
  const nonce = randomBytes(12), cipher = createCipheriv('aes-256-gcm', Buffer.from(key, 'base64'), nonce, { authTagLength: 16 });
...
export function unseal(key: string, id: string, requestId: string, direction: 'request' | 'response', box: string): any {
  if (typeof box !== 'string' || box.length > 24_000_000) throw Error('Invalid encrypted payload.');
  const data = Buffer.from(box, 'base64');
  if (data.length < 28) throw Error('Invalid encrypted payload.');
  const cipher = createDecipheriv('aes-256-gcm', Buffer.from(key, 'base64'), data.subarray(0, 12), { authTagLength: 16 });
  cipher.setAAD(aad(id, requestId, direction)); cipher.setAuthTag(data.subarray(-16));
...
export async function claim(link: Link, signal?: AbortSignal): Promise<Link> {
  const result = await invoke(link, { operation: 'claim', body: { name: 'LocalBot client' } }, signal);
