...
export const connectionRelay = process.env.LOCALBOT_RELAY_URL ?? "https://relay-five-lake.vercel.app";
export class RoutingPublisher {
  ...
  async start(file:string,relay:string,tunnel:string){
    ...
    await publish();
    let ready=false;
    for(let attempt=0;attempt<20&&this.generation===generation;attempt++){
      try{const response=await fetch(endpoint+'/rpc',{method:'POST',redirect:'error',headers:{'Content-Type':'application/json',Authorization:'Bearer '+ 'x'.repeat(43)},body:JSON.stringify({host,id:'probe',requestId:'probe',box:'probe'}),signal:AbortSignal.timeout(5000)});if(response.status===401){ready=true;break;}}catch{}
      await new Promise(resolve=>setTimeout(resolve,1000));
    }
    if(!ready)throw Error('The host connection is not reachable yet. Check internet access and try again.');
    if(this.generation!==generation)throw Error('Connection cancelled');this.host=host;this.timer=setInterval(()=>void publish().catch(()=>{}),60000);this.timer.unref();return host;
  }
