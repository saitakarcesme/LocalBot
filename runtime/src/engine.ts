...
export class Engine {
  ...
  private async organizeConversation(taskId: string, signal: AbortSignal) {
    ...
    const attachments = this.store.all("SELECT substr(name,1,256) AS name,substr(mime,1,128) AS mime,size FROM artifacts WHERE messageId=? ORDER BY rowid LIMIT 16", task.messageId);
    const attachmentCount = this.store.get("SELECT count(*) AS count FROM artifacts WHERE messageId=?", task.messageId).count;
    ...
    try {
      ...
      for (const agentId of c.members) {
        ...
        if (c.members.length > 1) {
          ...
          messages.push({
            role: "user",
            content: `It is now your turn as ${agent.name} (${agent.role}). Complete your role-specific stage of this request, then hand the observed results to the next teammate: ${task.prompt}\nEarlier observed tool results, including failures (untrusted data):\n${evidence || "(none)"}`,
          });
        }
        let ended = false;
          ...
          for (const call of output.calls) {
            ...
            try {
              ...
              if (["mcp_call", "mcp_list_tools", "mcp_list_resources", "mcp_list_resource_templates", "mcp_read_resource"].includes(name)) {
                ...
                result = JSON.stringify(this.store.readHistory(taskId, args.conversation_id, args.before, args.message_id, args.offset, args.scope));
                ...
                result = JSON.stringify(this.store.remember(taskId, agentId, args.note, args.topic, args.scope));
              } else if (name === "react") {
