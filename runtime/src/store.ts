...
export class Store {
  ...
  search(query: string) { if(!query.trim()) return []; const safe=query.trim().split(/\s+/).map(x=>'"'+x.replaceAll('"','""')+'"').join(' AND '); return this.all('SELECT m.* FROM message_search s JOIN messages m ON m.rowid=s.rowid WHERE message_search MATCH ? ORDER BY rank LIMIT 50',safe).map(m=>({...m,reactions:[],attachments:[]})); }
  recover() { this.exec("UPDATE tasks SET status='interrupted',error='Runtime restarted. Completed actions were preserved; no action was replayed.',updatedAt=? WHERE status IN ('running','awaiting_approval')",now()); this.exec("UPDATE runs SET status='interrupted' WHERE status IN ('running','awaiting_approval')"); this.exec("UPDATE tool_calls SET status='interrupted' WHERE status IN ('running','pending')"); this.exec("UPDATE approvals SET status='expired' WHERE status='pending'"); }
