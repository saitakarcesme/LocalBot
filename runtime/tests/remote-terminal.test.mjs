...
test('remote PTY supports input resize output replay protection and device revocation',{skip:process.platform!=='darwin'},async()=>{
 ...
 try{
  const {session}=await terminals.open('phone',root,'chat');terminals.handle('phone',{action:'resize',session,cols:91,rows:33});
  ...
  const resumed=await terminals.open('phone',root,'chat');assert.equal(resumed.session,session);assert.equal(resumed.sequence,1);
  assert.match(Buffer.from(terminals.handle('phone',{action:'poll',session:resumed.session,offset:0}).data,'base64').toString(),/PTY_CHECK/);
