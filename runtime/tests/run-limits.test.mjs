...
test('configured limits stop incomplete runs without claiming completion and permit longer tasks',async()=>{
 ...
 agent.maxSteps=6;store.saveAgent(agent);requests=0;const second=engine.enqueue(c.id,'Read the clock four times now');assert.equal((await finished(second.id)).status,'completed');assert.equal(requests,5);
 agent.maxSteps=5;store.saveAgent(agent);requests=0;const finalRound=engine.enqueue(c.id,'Read the clock four times and report the budget boundary');assert.equal((await finished(finalRound.id)).status,'completed_with_errors');assert.equal(requests,5);
